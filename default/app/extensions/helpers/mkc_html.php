...
class MkcHtml extends Html {
        ...
        if (is_array($attrs) OR empty($attrs)) {            
            if($loadAjax) {
                if(empty($attrs['class'])) {                    
                    $attrs['class'] = 'mkc-ajax mkc-spinner';
                } else {                    
                    if(!preg_match("/\bno-ajax\b/i", $attrs['class'])) {
                        $attrs['class'] = 'mkc-ajax '.$attrs['class'];
                    }                     
                    if(!preg_match("/\bno-spinner\b/i", $attrs['class'])) {
                        $attrs['class'] = 'mkc-spinner '.$attrs['class'];
                    }                 
                }                
            }                   
            if(!empty($attrs)) {
    ...
    public static function button($action, $text = NULL, $attrs = NULL, $icon='', $loadAjax = APP_AJAX) {
        if (is_array($attrs) OR empty($attrs)) {
            ...
            if(!preg_match("/\bmkc-text-bold\b/i", $attrs['class'])) {
                $attrs['class'] = $attrs['class'].' mkc-text-bold';
            }   
            if(!empty($attrs)) {
                $attrs = Tag::getAttrs($attrs);
            }            
     ...
     * 
     * @example echo MkcHtml::outLink('http://dailyscript.com.co', 'Enlace') Crea un enlace a esa url
     *
