...
class MkcMessage {
    ...
    public static function set($name, $msg, $audit=FALSE) {        
        ...
        if(self::has('mkc-messages')) {            
            self::$_contentMsj = Session::get('mkc-messages');                
        }        
        ...
        Session::set('mkc-messages', self::$_contentMsj);
        //Verifico si el mensaje se almacena como looger
        if($audit) {
            ($name=='success') ? MkcAudit::debug($msg) : MkcAudit::$name($msg);
        }            
    }
    ...
    public static function has() {
        return Session::has('mkc-messages') ?  TRUE : FALSE;
    }
    ...
    public static function toString() {
        //Asigno los mensajes almacenados en sesión en una variable temporal
        $tmp = self::has() ? Session::get('mkc-messages') : array();
        $msg = array();
        //Recorro los mensajes
        foreach($tmp as $item) {
            //Limpio los mensajes
