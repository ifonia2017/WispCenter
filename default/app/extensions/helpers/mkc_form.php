...
class MkcForm extends Form {
    ...
    protected static function _getValidationForm() {
        return '<script type="text/javascript"> $(function() { $("#'.self::$_name['id'].'").on("submit", function(){ este = $(this); before_send = este.attr("before-send"); after_send = este.attr("after-send"); confirmation = este.hasClass("mkc-confirm") ? true : false; val = true; if(before_send!=undefined) { try { val = eval(before_send); } catch(e) { } } if(!val) { return false; } val = validForm(este.attr("name"), confirmation); if(!val) { return false; } if(after_send!=undefined) { try { eval(after_send); } catch(e) { } } }); }); </script>'.PHP_EOL;
    }
    ...
    public static function open($action=null, $method='post', $attrs=null, $validate=false) {
        ...
        if( (preg_match("/\bmkc-validate\b/i", $attrs['class']) OR $validate) && !preg_match("/\bjs-remote\b/i", $attrs['class']) ) {           
    ...
    public static function hiddenKey($field, $value, $textKey='form_key') {
        ...
        $value = MkcSecurity::getKey($value, $textKey);
