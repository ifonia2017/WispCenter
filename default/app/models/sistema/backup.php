...
class Backup extends ActiveRecord {
    ...
    public static function createBackup($data, $path='', $database='') {
        ...
        if(!is_writable($path)) {
            ActiveRecord::rollbackTrans();
            MkcMessage::error('Error: BKP-CRE001. El directorio de las copias de seguridad no tiene permisos de escritura.');
            return false;
        }        
        ...
        if($resultado) {
            ActiveRecord::rollbackTrans();
            MkcMessage::error('Error: BKP-CRE002. Se ha producido un error al intentar crear una nueva copia de seguridad.');
            return false;            
        }        
        ...
        if($obj) {
            MkcAudit::debug("Se crea una copia de seguridad bajo la denominación: $obj->denominacion");
        }
        return $obj;         
    }
    ...
    public function before_save() {
        ...
        if($this->count($conditions)) {
            MkcMessage::error('Lo sentimos, pero ya existe una copia de seguridad con la misma denominación.');
            return 'cancel';
        }
    }
