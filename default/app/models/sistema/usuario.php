    ...
    public static function setUsuario($method, $data, $optData=null) {
        ...
        if(!empty($obj->id)) { //Si va a actualizar
            ...
            if(!empty($obj->oldpassword)) { //Si cambia de claves
                if(empty($obj->password) OR empty($obj->repassword)) {
                    MkcMessage::error("Indica la nueva contraseña");
                    return false;
                }
                $obj->oldpassword = md5(sha1($obj->oldpassword));
                if($obj->oldpassword !== $old->password) {
                    MkcMessage::error("La contraseña anterior no coincide con la registrada. Verifica los datos e intente nuevamente");
                    return false;
                }
            }                       
        }
        ...
        if( (!empty($obj->password) && !empty($obj->repassword) ) OR ($method=='create')  ) { 
            if($method=='create' && (empty($obj->password))) {
                MkcMessage::error("Indica la contraseña para el inicio de sesión");
                return false;
            }
            $obj->password = md5(sha1($obj->password));
            $obj->repassword = md5(sha1($obj->repassword));            
            if($obj->password !== $obj->repassword) {
                MkcMessage::error('Las contraseñas no coinciden. Verifica los datos e intenta nuevamente.');
                return 'cancel';
            }
        } else {
        ...
        $rs = $obj->$method();
        if($rs) {
            ($method == 'create') ? MkcAudit::debug("Se ha registrado el usuario $obj->login en el sistema") : MkcAudit::debug("Se ha modificado la información del usuario $obj->login");
        }
        return ($rs) ? $obj : FALSE;
    }
