...
class Sistema {
    ...
    public function getDesfragmentacion($tabla) {
        if(in_array($tabla, $this->_tables)) {
            ...
            if($rs) {
                MkcAudit::info("Se ha realizado el mantenimiento de desfragmentación a la tabla $tabla");
            } else {
                MkcAudit::error("Se ha generado un error al realizar el mantenimiento de desfragmentación a la tabla $tabla");
            }
            return $rs;
        } else {
            return FALSE;
    ...
    public function getVaciadoCache($tabla) {
        if(in_array($tabla, $this->_tables)) {
            ...
            if($rs) {
                MkcAudit::info("Se ha realizado el mantenimiento de vaciado del caché a la tabla $tabla");
            } else {
                MkcAudit::error("Se ha generado un error al realizar el mantenimiento de vaciado del caché a la tabla $tabla");
            }
            return $rs;
        } else {
            return FALSE;
    ...
    public function getReparacionTabla($tabla) {
        if(in_array($tabla, $this->_tables)) {
            ...
            if($rs) {
                MkcAudit::info("Se ha realizado el mantenimiento de reparación a la tabla $tabla");
            } else {
                MkcAudit::error("Se ha generado un error al realizar el mantenimiento de reparación a la tabla $tabla");
            }
            return $rs;
        } else {
            return FALSE;
    ...
    public static function getLogger($fecha, $page) {
        ...
        $result = MkcUtils::orderArray($new_log, 'item', TRUE);                
        //Pagino el array
        $paginate = new MkcPaginate();
    ...
    public static function getAudit($fecha, $page=0) {
        $audit = MkcRead::file('audit'.$fecha);
        //Armo un nuevo array para ordenarlos 
        $contador = 0;
        $new_log = array();
        if(!empty($audit)) {
    ...
    public static function setRoutes($data=null) {
        ...
        $rs = MkcConfig::write('routes', $data, 'routes');
        if($rs) {
            MkcAudit::info('Se ha actualizado el archivo de enrutamiento interno del sistema');
        }
        return $rs;
    }
    ...
    public static function reset() {
        ...
        MkcAudit::info('Se han restaurado los archivos de configuración, enrutamiento y de conexión del sistema');
        //@TODO revisar esto;
        return true;
    }
}
