...
class WideImage_vendor_de77_BMP
{
	public static function imagebmp(&$img, $filename = false)
	{
		...
		$header['file_size']		= self::mkcord($size);
		$header['reserved']			= self::mkcord(0);
		$header['bitmap_data']		= self::mkcord(54);
		$header['header_size']		= self::mkcord(40);
		$header['width']			= self::mkcord($wid);
		$header['height']			= self::mkcord($hei);
		$header['planes']			= self::word(1);
		$header['bits_per_pixel']	= self::word(24);
		$header['compression']		= self::mkcord(0);
		$header['data_size']		= self::mkcord(0);
		$header['h_resolution']		= self::mkcord(0);
		$header['v_resolution']		= self::mkcord(0);
		$header['colors']			= self::mkcord(0);
		$header['important_colors']	= self::mkcord(0);
	...
	public static function imagecreatefromstring($data)
	{
		...
		if ($bps < 9)
		{
			for ($i=0; $i<$colors; $i++)
			{
				$palette[] = self::unmkcord(substr($data, $pos, 4));
				$pos += 4;
			}
		}
		else
	...
	private static function unmkcord($n)
	{
		$r = unpack("V", $n);
		return $r[1];
	}
	
	private static function mkcord($n)
	{
		return pack("V", $n);
	}
