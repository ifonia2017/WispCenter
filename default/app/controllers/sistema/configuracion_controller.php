...
class ConfiguracionController extends BackendController {
    ...
    public function index() {
        if(Input::hasPost('application') && Input::hasPost('custom')) {
            try {
                ...
                MkcMessage::valid('El archivo de configuración se ha actualizado correctamente!');
        ...
        $this->config = MkcConfig::read('config', '', true);
    ...
    public function routes() {
        if(Input::hasPost('routes')) {
            try {
                ...
                MkcMessage::valid('El archivo de enrutamiento se ha actualizado correctamente!');
            } catch(KumbiaException $e) {
                MkcMessage::error('Oops!. Se ha realizado algo mal internamente. <br />Intentalo de nuevo!.');
            }
            Input::delete('routes');
        }
        $this->routes = MkcConfig::read('routes', '', true);
    ...
    public function test() {
        if(!Input::isAjax()) {
            MkcMessage::error('Acceso incorrecto para la verificación del sistema.');
            return MkcRedirect::toRoute('module: dashboard', 'controller: index');
        }
        if(!Input::hasPost('development') OR !(Input::hasPost('production')) ) {
            MkcMessage::error('Oops!. No hemos recibido algún parámetro de configuración.');
        } else {
